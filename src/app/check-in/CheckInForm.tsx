...
import type { DailyCheckinRecord } from "@/lib/health/schema";
...
function toTextareaValue(value: DailyCheckinRecord["symptoms"]) {
  return Array.isArray(value)
    ? value.filter((item): item is string => typeof item === "string").join("\n")
    : "";
}

export function CheckInForm({ initialCheckin = null }: CheckInFormProps) {
            ...
            <section className="space-y-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <SelectField
                  label="컨디션 점수"
                  name="conditionScore"
                  defaultValue={String(initialCheckin?.condition_score ?? 5)}
                  options={scoreOptions}
                />
                <SelectField
                  label="수면의 질"
                  name="sleepQuality"
                  defaultValue={String(initialCheckin?.sleep_quality ?? 5)}
                  options={scoreOptions}
                />
                <SelectField
                  label="스트레스"
                  name="stressLevel"
                  defaultValue={String(initialCheckin?.stress_level ?? 5)}
                  options={scoreOptions}
                />
                <SelectField
                  label="증상 심각도"
                  name="symptomSeverity"
                  defaultValue={String(initialCheckin?.symptom_severity ?? 0)}
                  options={symptomSeverityOptions}
                />
                <SelectField
                  label="식욕"
                  name="appetite"
                  defaultValue={initialCheckin?.appetite ?? ""}
                  options={[
                    { value: "", label: "선택" },
                ...
                <SelectField
                  label="소화 상태"
                  name="digestion"
                  defaultValue={initialCheckin?.digestion ?? ""}
                  options={[
                    { value: "", label: "선택" },
                    { value: "normal", label: "보통" },
              ...
              <label className="flex items-start gap-3 rounded-lg border border-[var(--border)] bg-[var(--background)] px-4 py-3 text-sm text-[var(--foreground)]">
                <input
                  type="checkbox"
                  name="exercisedToday"
                  defaultChecked={initialCheckin?.exercised_today ?? false}
              ...
              <div className="grid gap-4 border-t border-[var(--border)] px-4 py-4 sm:grid-cols-2">
                <div className="sm:col-span-2">
                  <TextField
                    ...
                    defaultValue={toTextareaValue(initialCheckin?.symptoms ?? [])}
                  />
                </div>
                <TextField
                  label="물 섭취량 (컵)"
                  ...
                  placeholder="6"
                  defaultValue={
                    initialCheckin?.water_intake == null
                      ? ""
                      : String(initialCheckin.water_intake)
                  }
                />
                <div className="sm:col-span-2">
                  <TextField
                    ...
                    defaultValue={initialCheckin?.notes ?? ""}
                  />
                </div>
              </div>
            </details>
